...
Content here.`);
		...
		test('should add property to all specified entities', async () => {
			const targetEntities = plugin.entityScanner.getEntityInstances().filter(e => e.entityType === 'Person');
		...
		test('should handle different value types correctly', async () => {
			const targetEntities = plugin.entityScanner.getEntityInstances().slice(0, 1); // Just one entity for simplicity
		...
		test('should preserve existing content structure', async () => {
			const targetEntities = plugin.entityScanner.getEntityInstances().slice(0, 1);
...
Content here.`);
		...
		test('should detect schema drift', async () => {
			...
			const driftEntities = plugin.entityScanner.getEntityInstances().filter(e => e.missingProperties.length > 0);
