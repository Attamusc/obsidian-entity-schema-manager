...
describe('Entity Discovery Integration', () => {
  ...
  describe('scanEntities', () => {
    test('should discover all matching entities', async () => {
      ...
      expect(plugin.entityInstances).toHaveLength(2);
      expect(plugin.entityInstances.some(e => e.entityType === 'Person')).toBe(true);
      expect(plugin.entityInstances.some(e => e.entityType === 'Team')).toBe(true);
      expect(plugin.entityInstances.some(e => e.file.path === 'atlas/notes/john.md')).toBe(true);
      expect(plugin.entityInstances.some(e => e.file.path === 'atlas/notes/team.md')).toBe(true);
    });
    ...
    test('should identify missing properties', async () => {
      ...
      expect(plugin.entityInstances).toHaveLength(0);
    });
    ...
    test('should match only first applicable schema', async () => {
      ...
      expect(plugin.entityInstances[0].entityType).toBe('Person');
    });
    ...
    test('should handle large numbers of files efficiently', async () => {
      ...
      expect(plugin.entityInstances).toHaveLength(fileCount);
  ...
  describe('Entity Type Assignment', () => {
    test('should assign correct entity types based on schemas', async () => {
      ...
      const personEntity = plugin.entityInstances.find(e => e.file.path === 'atlas/notes/person.md');
      const teamEntity = plugin.entityInstances.find(e => e.file.path === 'atlas/notes/team.md');
  ...
  describe('Schema Matching Priority', () => {
    test('should match schemas in order of definition', async () => {
      ...
      expect(plugin.entityInstances).toHaveLength(1);
      expect(plugin.entityInstances[0].entityType).toBe('Ambiguous');
    });
  });
});